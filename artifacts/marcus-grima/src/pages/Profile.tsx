...
const goals = [
  ...
  { label: 'Current Phase', value: 'Hypertrophy Block' },
];
...
export const Profile = ({ setPage }: ProfileProps) => {
  const [notifications, setNotifications] = useState(true);

  return (
    <div className="min-h-screen bg-[#0A0A0A] text-foreground pb-24 overflow-x-hidden">
      {/* Header */}
      <header className="px-5 py-4 sticky top-0 z-30 bg-[#0A0A0A]/80 backdrop-blur-md border-b border-white/5 flex justify-between items-center">
      ...
      <motion.div
        className="px-5 pt-6 pb-12 flex flex-col gap-8"
      ...
      >
        ...
        <motion.div variants={itemVariants} className="flex flex-col items-center text-center">
          ...
          <h2 className="text-2xl font-bold tracking-[0.15em] uppercase mb-1">Marcus</h2>
          ...
          <div className="flex items-center gap-2 bg-primary/10 border border-primary/20 px-4 py-2">
            ...
            <span className="text-[11px] font-bold tracking-wider text-primary uppercase">Hypertrophy Block · Week 6</span>
          </div>
        </motion.div>
        ...
        <motion.section variants={itemVariants} className="space-y-4">
          ...
          <div className="bg-[#111111] border border-white/5 rounded-sm overflow-hidden">
            {settingsItems.map((item, i) => (
              <div
                key={item.label}
                className={`flex justify-between items-center px-5 py-4 ${i < settingsItems.length - 1 ? 'border-b border-white/5' : ''}`}
              >
                ...
                {item.action === 'toggle' ? (
                  ...
                  >
                    <span
                      className={`absolute top-1 w-4 h-4 rounded-full bg-foreground transition-transform ${notifications ? 'translate-x-6' : 'translate-x-1'}`}
                    />
                  </button>
                ) : (
        ...
        <motion.section variants={itemVariants}>
          <button className="w-full flex items-center justify-center gap-3 py-4 border border-white/10 text-sm font-bold tracking-[0.15em] uppercase text-foreground/50 hover:text-red-400 hover:border-red-500/20 transition-colors">
            <LogOut size={16} />
            Log Out
          </button>
        </motion.section>
      ...
      <BottomNav activePage="profile" onNavigate={setPage} />
    </div>
  );
};

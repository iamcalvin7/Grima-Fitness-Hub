...
import {
  ...
  CheckCircle2, XCircle, ChevronLeft, X, Check,
} from 'lucide-react';
...
const upcoming = [
  { id: 1, date: 'THURSDAY, 24 JULY', time: '07:00 AM', duration: '60 MIN', location: 'Marcus Grima Studio, London', status: 'CONFIRMED' },
  { id: 2, date: 'SATURDAY, 26 JULY', time: '08:30 AM', duration: '60 MIN', location: 'Marcus Grima Studio, London', status: 'CONFIRMED' },
  { id: 3, date: 'TUESDAY, 29 JULY',  time: '07:00 AM', duration: '60 MIN', location: 'Marcus Grima Studio, London', status: 'PENDING'   },
];

const past = [
  { id: 4, date: 'TUESDAY, 22 JULY',   time: '07:00 AM', duration: '55 MIN', location: 'Marcus Grima Studio, London', status: 'COMPLETED', exercises: 7 },
  { id: 5, date: 'SATURDAY, 19 JULY',  time: '09:00 AM', duration: '60 MIN', location: 'Marcus Grima Studio, London', status: 'COMPLETED', exercises: 9 },
  { id: 6, date: 'THURSDAY, 17 JULY',  time: '07:00 AM', duration: '60 MIN', location: 'Marcus Grima Studio, London', status: 'COMPLETED', exercises: 8 },
  { id: 7, date: 'TUESDAY, 15 JULY',   time: '07:00 AM', duration: '45 MIN', location: 'Marcus Grima Studio, London', status: 'CANCELLED', exercises: 0 },
  { id: 8, date: 'SATURDAY, 12 JULY',  time: '09:00 AM', duration: '60 MIN', location: 'Marcus Grima Studio, London', status: 'COMPLETED', exercises: 6 },
  { id: 9, date: 'THURSDAY, 10 JULY',  time: '07:00 AM', duration: '55 MIN', location: 'Marcus Grima Studio, London', status: 'COMPLETED', exercises: 7 },
];

/* ── Availability data ─────────────────────────────────────────────────── */

// Generate available slots relative to today (July 24 2026)
const TODAY = new Date(2026, 6, 24); // July 24
...
function buildAvailability(): Map<string, DaySlots> {
  ...
  // Pre-booked times to make it realistic
  const bookedMap: Record<string, string[]> = {
    '2026-07-24': ['07:00', '08:00'],
    '2026-07-25': ['06:00', '17:00', '19:00'],
    '2026-07-26': ['09:00'],
    '2026-07-28': ['07:00', '12:00', '18:00'],
    '2026-07-29': ['07:00'],
    '2026-07-30': ['06:00', '08:00', '13:00'],
    '2026-07-31': ['07:00', '17:00', '19:00'],
    '2026-08-01': ['08:00', '11:00'],
    '2026-08-04': ['06:00', '12:00'],
    '2026-08-05': ['17:00'],
    '2026-08-06': ['09:00', '10:00'],
    '2026-08-11': ['07:00', '08:00', '18:00'],
  };
  ...
  for (let i = 0; i < 28; i++) {
    ...
    if (dow === 0) { map.set(key, { slots: [] }); continue; } // Sunday off
    ...
    map.set(key, {
      slots: rawSlots.map(t => ({
        time: t,
        status: booked.includes(t) ? 'booked' : 'available',
      })),
    });
  }
...
function BookingSheet({ onClose }: { onClose: () => void }) {
  ...
  const calDays = useMemo(() => {
    ...
    const firstDow = (calMonth.getDay() + 6) % 7; // Mon=0
    ...
    >
      ...
      <div className="flex items-center justify-between px-6 py-5 border-b border-white/8 shrink-0">
        <div className="flex items-center gap-3">
          {step !== 'date' && step !== 'done' && (
            <button
              onClick={() => { if (step === 'time') setStep('date'); if (step === 'confirm') setStep('time'); }}
        ...
        <AnimatePresence mode="wait">
            ...
            <motion.div key="date" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="px-6 py-5">
              ...
              <div className="flex items-center justify-between mb-4">
                <button onClick={prevMonth} disabled={!canPrevMonth}
                  className="w-8 h-8 flex items-center justify-center text-foreground/40 hover:text-foreground disabled:opacity-20 transition-colors">
                  <ChevronLeft size={16} />
                </button>
                <span className="text-sm font-bold tracking-[0.12em]">
                  {MONTH_NAMES[calMonth.getMonth()]} {calMonth.getFullYear()}
                </span>
                <button onClick={nextMonth} className="w-8 h-8 flex items-center justify-center text-foreground/40 hover:text-foreground transition-colors">
              ...
              <div className="grid grid-cols-7 mb-2">
                {DAY_NAMES.map(d => (
                  <div key={d} className="text-center text-[9px] font-bold tracking-widest text-foreground/25 uppercase py-1">{d}</div>
                ))}
              </div>
          ...
          {step === 'time' && (
            <motion.div key="time" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="px-6 py-5">
              ...
              {/* All slots grid — showing booked ones as unavailable */}
                ...
                {selectedSlots.map(({ time, status }) => {
                  ...
                  return (
                    <button
                      ...
                      className={`py-3 rounded-sm border text-xs font-bold tracking-wider transition-all ${
                        isSel
                          ? 'bg-primary border-primary text-white'
                          : isAvail
                          ? 'border-white/12 text-foreground hover:border-primary/50 hover:bg-primary/8'
                          : 'border-white/4 text-foreground/15 cursor-default line-through'
                      }`}
                    >
                      {fmt12(time)}
                    </button>
              ...
              <p className="text-[9px] text-foreground/30 mt-4 font-medium">
                All sessions are 60 minutes · Marcus Grima Studio, London
              </p>
          ...
          {step === 'confirm' && selectedDate && selectedTime && (
            <motion.div key="confirm" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="px-6 py-5">
              <div className="bg-[#0D0D0D] border border-white/8 rounded-sm p-5 mb-6">
                ...
                <div className="space-y-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-[9px] font-bold tracking-widest text-foreground/35 uppercase">Date</p>
                      <p className="text-sm font-bold mt-0.5">{selectedDateStr}</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <Timer size={15} className="text-primary mt-0.5 shrink-0" />
                    <div>
                      <p className="text-[9px] font-bold tracking-widest text-foreground/35 uppercase">Time</p>
                      <p className="text-sm font-bold mt-0.5">{fmt12(selectedTime)} · 60 min</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <MapPin size={15} className="text-primary mt-0.5 shrink-0" />
                    <div>
                      <p className="text-[9px] font-bold tracking-widest text-foreground/35 uppercase">Location</p>
                      <p className="text-sm font-bold mt-0.5">Marcus Grima Studio, London</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <p className="text-xs text-foreground/40 leading-relaxed mb-6">
                Marcus will receive your request and confirm shortly. You'll be notified once confirmed.
              </p>

              <button
                onClick={() => setStep('done')}
                className="w-full bg-primary hover:bg-primary/90 py-4 text-white font-bold tracking-[0.15em] uppercase text-sm transition-colors"
              >
                Confirm Booking
              </button>
            </motion.div>
          ...
          {step === 'done' && selectedDate && selectedTime && (
            <motion.div key="done" initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} className="px-6 py-10 flex flex-col items-center text-center">
              ...
              <p className="text-xs text-foreground/35 leading-relaxed max-w-[260px]">
                Marcus will confirm your session within a few hours. You'll get a notification once it's locked in.
              </p>
              <button
                onClick={onClose}
                className="mt-10 w-full border border-white/12 hover:border-white/25 py-4 text-sm font-bold tracking-[0.15em] uppercase text-foreground/60 hover:text-foreground transition-colors"
              >
                Done
              </button>
            </motion.div>
                ...
                <motion.div key={s.id} variants={itemVariants}>
                  <div className="bg-[#111111] border-l-2 border-l-primary rounded-r-sm p-5 h-full">
                    <div className="flex justify-between items-start mb-3">
                      <p className="text-[10px] font-bold tracking-widest text-foreground/40 uppercase">Personal Training</p>
